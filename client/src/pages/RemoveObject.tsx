...
import { Scissors, Sparkles, Lock, Upload, Download } from 'lucide-react';
...
export default function RemoveObject() {
  ...
  const isSubmitDisabled =
    loading || !file || !validateObjectName(object).isValid;
  ...
  const onSubmitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const objectValidation = validateObjectName(object);
    if (!objectValidation.isValid) {
      toast.error(objectValidation.message!);
      return;
    }
    ...
    try {
      ...
      const { data } = await axios.post('/api/ai/object', formData, {
        headers: {
      ...
      if (data.success) {
        ...
        toast.success('요소 제거가 완료되었습니다!');
      } else {
        toast.error(data.message || '요소 제거에 실패했습니다.');
      }
    } catch (err: unknown) {
      console.error('요소 제거 에러:', err);

      if (isAxiosError(err) && err.response) {
        const statusCode = err.response.status;
        const errorData = err.response.data;

        let errorMessage = '요소 제거에 실패했습니다.';
        ...
        switch (statusCode) {
          ...
          case 403:
            if (errorData?.message) {
              ...
              errorMessage =
                '요소 제거는 프리미엄 플랜에서만 이용 가능합니다. 업그레이드해주세요.';
            }
            break;
          case 413:
            errorMessage =
      ...
      >
        ...
        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-blue-700 font-medium">프리미엄 기능</p>
          </div>
          <p className="text-xs text-blue-600 mt-1">
            요소 제거는 프리미엄 플랜에서만 이용 가능합니다.
          </p>
        </div>
        ...
        <div className="mt-6">
          <label className="text-sm font-medium text-gray-700">
            제거할 요소 이름 <span className="text-red-500">*</span>
          </label>
          <p className="mt-1 text-xs text-gray-500">
            이미지에서 제거하고 싶은 사물의 이름을 입력하세요.
          </p>
          <textarea
            onChange={handleObjectChange}
            rows={3}
            value={object}
            className="w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors resize-none"
            placeholder="예: 시계, 나무, 사람"
            required
            disabled={loading}
            maxLength={50}
          />
          <div className="mt-1 text-xs text-gray-500 flex justify-between">
            <span>최대 3개 단어까지 입력 가능</span>
            <span>{object.length}/50</span>
          </div>
        </div>
        ...
        <button
          ...
          className={`w-full flex justify-center items-center gap-2 px-4 py-2 mt-6 text-sm rounded-lg transition-all ${
            isSubmitDisabled
              ...
              : 'bg-gradient-to-r from-[#471df6] to-[#4a7aff] text-white hover:from-[#3d17d4] hover:to-[#4169e1] cursor-pointer'
          }`}
        >
          {loading ? (
            <>
      ...
      <div className="w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold">요소 제거 결과</h1>
          </div>
          {content && (
            <button
          ...
          <div className="flex-1 flex justify-center items-center">
            <div className="text-sm flex flex-col items-center gap-5 text-gray-400">
              ...
              <p className="text-center">
                이미지를 업로드하고 제거할 요소 이름을 입력한 후<br />
                '요소 지우기'를 클릭해 시작하세요.
              </p>
            </div>
          </div>
